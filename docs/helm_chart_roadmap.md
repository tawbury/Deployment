...
| 항목 | 내용 |
|------|------|
| **목적** | 현재 K8s 배포가 "일단 작동하는" 상태 확보 |
| **해결하려는 문제** | PVC 미마운트, Secret 수동 관리, 환경 변수 불완전 |
| **절대 하지 말아야 할 것** | 새로운 아키텍처 도입, Helm 전환 시도, 구조 변경 |
| **다음 Phase 진입 조건** | Pod Running 24시간+ / Pod 재시작 시 데이터 보존 / /health 정상 / JSONL 생성 확인 |

### Phase 1: 실행 계약 정의

| 항목 | 내용 |
|------|------|
| **목적** | 앱-컨테이너-K8s 간 "계약"을 문서화하고 검증 절차 수립 |
| **해결하려는 문제** | 계층 간 암묵적 의존성, 디버깅 시 원인 파악 어려움 |
| **절대 하지 말아야 할 것** | 계약 없이 Helm 템플릿 작성, 문서화 없이 설정 변경 |
| **다음 Phase 진입 조건** | 경로/환경 변수/포트 계약 문서 완료 / 3단계 검증 체크리스트 완료 / 계약 위반 테스트 존재 |
